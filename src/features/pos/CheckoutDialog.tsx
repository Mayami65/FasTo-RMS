...
export function CheckoutDialog({
  ...
  useEffect(() => {
    if (open) {
      setAmountTendered(initialValues?.amountTendered?.toString() || "");
      ...
      setCustomerLabel(initialValues?.customerName || "");
      setHpDuration("TWO_WEEKS");
      setMomoTransactionId(initialValues?.momoTransactionId || "");
      setMomoProvider(initialValues?.momoProvider || "");
      setSelectedDiscountId(
        initialValues?.couponId ? initialValues.couponId.toString() : "none",
      );
      setError(null);
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (
        ...
        !customerSelectRef.current?.contains(e.target as Node)
      ) {
        setShowResults(false);
        setCustomerId((prev) => {
          if (!prev && !isQuickAdd && paymentMethod !== "HIRE_PURCHASE")
            return "none";
          return prev;
        });
      }
  ...
  const isValid =
    (isHP
      ? !!customerId && customerId !== "none" && numericAmount >= 0
      : numericAmount >= finalTotal || isHP) &&
    ...
    try {
      ...
      if (exists) {
        ...
        if (result.success) {
          ...
          setCustomerLabel(addedCustomer.name || "Walk-in Customer");
          setRecentCustomers((prev) =>
            [addedCustomer, ...prev.filter((c) => c.id !== result.id)].slice(
              0,
              5,
            ),
          );
  ...
  const handleConfirm = async () => {
    ...
    const selectedCustomer =
      ...
      recentCustomers.find((c) => c.id.toString() === customerId);
    const selectedCustomerName =
      selectedCustomer?.name ||
      customerLabel ||
      (customerId === "none" ? "Walk-in Customer" : "Selected Customer");
    ...
    try {
      await onConfirm({
        ...
        discount_amount:
          appliedDiscountAmount > 0 ? appliedDiscountAmount : undefined,
      ...
      <DialogContent className="sm:max-w-[400px]">
        ...
        <div className="space-y-4 py-4 max-h-[80vh] overflow-y-auto px-1">
          ...
          <div className="flex justify-between items-center text-xl font-bold">
            ...
            <div className="text-right">
              {appliedDiscountAmount > 0 && (
                <p className="text-sm text-green-600 line-through font-normal opacity-50">
                    ...
                    <div className="p-2 bg-amber-50 border border-amber-100 rounded text-[10px] text-amber-800 flex items-center gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          setCustomerLabel(
                            duplicateCustomer.name || "Walk-in Customer",
                          );
                          setIsQuickAdd(false);
                        }}
                      >
                ...
                <>
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "ArrowDown") {
                            ...
                            setHighlightedIndex((prev) =>
                              Math.min(prev + 1, customers.length),
                            );
                            ...
                            if (highlightedIndex === 0) {
                              ...
                              setCustomerId(
                                customers[highlightedIndex - 1].id.toString(),
                              );
                              setCustomerLabel(
                                customers[highlightedIndex - 1].name ||
                                  "Walk-in Customer",
                              );
                            }
                            setShowResults(false);
                      ...
                      {customerId && customerId !== "none" && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setCustomerId(
                              paymentMethod === "HIRE_PURCHASE" ? "" : "none",
                            );
                            setCustomerLabel(
                        ...
                        <div className="max-h-[250px] overflow-y-auto py-1">
                          {paymentMethod !== "HIRE_PURCHASE" && (
                            ...
                            >
                              ...
                              <Badge
                                variant="outline"
                                className="text-[9px] font-black tracking-widest uppercase h-4"
                              >
                                DEFAULT
                              </Badge>
                            </div>
                          ...
                          {isSearching ? (
                            ...
                            customers.map((c, idx) => (
                              ...
                              >
                                ...
                                <span className="text-[10px] opacity-40 uppercase font-black tracking-tighter">
                                  Gold Customer
                                </span>
                              </div>
                            ))
                            ...
                            <div className="px-4 py-8 text-center">
                              <p className="text-xs text-slate-400 font-medium">
                                No results found for "{customerSearch}"
                              </p>
                              <Button
                                variant="link"
                  ...
                  {recentCustomers.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      <span className="text-[9px] uppercase font-black text-slate-400 tracking-widest w-full mb-0.5">
                        Focus: Recent
                      </span>
                      {recentCustomers.map((c) => (
                        <button
                          ...
                          onClick={() => {
                            setCustomerId(c.id.toString());
                            setCustomerLabel(c.name || "Walk-in Customer");
                          }}
                          className={cn(
                  ...
                  {!isQuickAdd && (
                    <div className="flex items-center justify-between p-2 bg-blue-50/50 rounded-lg border border-blue-100">
                      <p className="text-[9px] text-blue-600 font-bold italic flex items-center gap-1">
                        <kbd className="bg-blue-100 px-1 rounded text-blue-700 not-italic">
                          F2
                        </kbd>{" "}
                        Focus Search
                      </p>
              ...
              <SelectContent>
                ...
                {discounts.map((d) => (
                  <SelectItem key={d.id} value={d.id!.toString()}>
                    {d.name} (
                    {d.type === "PERCENTAGE" ? `${d.value}%` : `GH₵ ${d.value}`}
                    )
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          <div className="p-4 rounded-md text-center bg-blue-50 text-blue-800">
            ...
            <p className="text-2xl font-bold">
              GH₵ {(finalTotal - numericAmount).toFixed(2)}
            </p>
          </div>
        </div>
