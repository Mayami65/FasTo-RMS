...
export function ReceiptPreviewDialog({ open, onClose, sale, isHistory }: ReceiptPreviewDialogProps) {
    if (!sale) return null;
        ...
        <Dialog open={open} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[450px]">
                <DialogHeader>
                    ...
                    <DialogTitle>{isHistory ? 'Sale Details' : 'Receipt Preview'}</DialogTitle>
                    <DialogDescription>
                        {isHistory ? `Detail view for transaction ${sale.id}` : 'Review the transaction details before printing or closing.'}
                    </DialogDescription>
                </DialogHeader>
