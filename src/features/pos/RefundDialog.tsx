...
export function RefundDialog({
  ...
  const handleSearchByCustomerName = async () => {
    ...
    try {
      const results =
        await window.api.searchSalesByCustomerName(customerNameInput);
  ...
  const handleProcessRefund = async () => {
    ...
    try {
      ...
      if (result.success) {
        alert(
          `Refund completed. Amount refunded: GH₵ ${(result.refundAmount || 0).toFixed(2)}`,
        );
        onRefundSuccess();
        handleClose();
      } else {
      ...
      <DialogContent className="sm:max-w-[700px]">
        ...
        <div className="space-y-4 py-4 max-h-[80vh] overflow-y-auto px-1">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="grid grid-cols-[1fr_auto] gap-2 items-end">
              ...
              <Button
                onClick={handleSearchByCustomerName}
                disabled={loadingSale}
                variant="secondary"
              >
                Search
                ...
                {foundSales.map((sale) => (
                  ...
                  >
                    <div className="text-left">
                      <p className="font-black text-primary group-hover:underline">
                        Sale #{sale.id}
                      </p>
            ...
            <div className="space-y-4 border rounded-md p-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {new Date(saleData.sale.timestamp).toLocaleString()} -{" "}
                    {saleData.sale.payment_method}
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-sm text-muted-foreground">
                  ...
                  <p className="font-semibold">
                    GH₵ {(saleData.refunded_total || 0).toFixed(2)}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3 max-h-[350px] overflow-y-auto pr-1">
                {saleData.items.map((item: any) => (
                  <div
                    key={item.id}
                    className="grid grid-cols-[1fr_auto_auto] gap-3 items-center border rounded p-3"
                  >
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Qty Sold: {item.quantity} | Refunded:{" "}
                        {item.refunded_quantity} | Available:{" "}
                        {item.refundable_quantity}
                      </p>
                    </div>
                    <p className="text-sm font-medium">
                      GH₵ {item.price_at_sale.toFixed(2)}
                    </p>
                    <Input
                      type="number"
                      min={0}
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={processing || !saleData?.sale}
          >
