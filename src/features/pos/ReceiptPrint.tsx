...
export const ReceiptPrint = forwardRef<HTMLDivElement, ReceiptPrintProps>(
  ({ sale, isInline }, ref) => {
    ...
    const content = (
      <div
        ref={ref}
        className={`p-2 font-mono ${receiptWidth} mx-0 text-[12px] leading-tight text-black bg-white ${isInline ? "border shadow-sm" : "print-only no-print-screen"}`}
      >
        <style
          dangerouslySetInnerHTML={{
            __html: `
                ...
                @media print {
                  ...
                  .print-only { display: block !important; position: static !important; width: 80mm !important; margin: 0 !important; padding: 4mm 4mm 6mm 4mm !important; box-sizing: border-box !important; font-size: 11px !important; line-height: 1.12 !important; }
                  .print-only * { box-sizing: border-box !important; }
                  /* Allow flex children to shrink and wrap so long text doesn't overflow the paper */
                  .print-only .flex { display: flex !important; flex-wrap: wrap !important; }
                  .print-only .flex > * { min-width: 0 !important; }
                  .print-only .truncate { white-space: normal !important; overflow-wrap: anywhere !important; word-break: break-word !important; }
                  /* Ensure right-aligned fields still wrap instead of overflowing */
                  .print-only .text-right { text-align: right !important; }
                }
            `,
          }}
        />
        ...
        <div className="text-center mb-2">
          ...
          {settings.receiptHeader && (
            <p className="text-[10px] italic">{settings.receiptHeader}</p>
          )}
          <p className="text-[10px]">{settings.storeAddress}</p>
          <p className="text-[10px]">Tel: {settings.storePhone}</p>
          {settings.showStoreEmail === "true" && settings.storeEmail && (
            <p className="text-[10px]">{settings.storeEmail}</p>
          )}
          {settings.taxId && (
            <p className="text-[10px]">TIN: {settings.taxId}</p>
          )}
        </div>
        ...
        <div className="space-y-1 mb-2">
          ...
          {sale.items.map((item, index) => (
            <div key={index} className="text-[10px] py-0.5">
              <div className="flex justify-between">
                <span className="font-bold truncate pr-2 uppercase italic">
        ...
        <div className="space-y-1 text-[11px]">
          <div className="flex justify-between font-bold">
            <span>SUBTOTAL</span>
            <span>
              GH₵ {(Number(sale.subtotal || sale.totalAmount) || 0).toFixed(2)}
            </span>
          </div>
              ...
              <div className="flex justify-between font-bold">
                ...
                <span>
                  ...
                  {(
                    ...
                    (Number(sale.amountTendered) || 0)
                  ).toFixed(2)}
                </span>
              </div>
              <p className="text-[8px] mt-1 italic">
                Title remains with FasTo until full payment.
              </p>
              <br />
            </div>
          )}
